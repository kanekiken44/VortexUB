...
@Vortex.on(admin_cmd(pattern="fstat ?(.*)"))
@Vortex.on(sudo_cmd(pattern="fstat ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@Vortex.on(admin_cmd(pattern="fedinfo ?(.*)"))
@Vortex.on(sudo_cmd(pattern="fedinfo ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    ...
    async with borg.conversation(bot) as conv:
        try:
            ...
            audio = await conv.get_response()
            await ok.edit(audio.text + "\n\nFedInfo Collected By Vortexuserbot")
        except YouBlockedUserError:
            await ok.edit("**Error**\n `Unblock` @MissRose_Bot `and try again!")
