...
from Vortex.plugins.sql_helper.mute_sql import is_muted, mute, unmute
from Vortex.utils import admin_cmd


@Vortex.on(admin_cmd(outgoing=True, pattern=r"gmute ?(\d+)?"))
@Vortex.on(sudo_cmd(allow_sudo=True, pattern=r"gmute ?(\d+)?"))
async def startgmute(event):
    ...
    try:
        ...
        await eor(event, "Silence now. **Successfully gmuted that Noob**")


@Vortex.on(admin_cmd(outgoing=True, pattern=r"ungmute ?(\d+)?"))
@Vortex.on(sudo_cmd(allow_sudo=True, pattern=r"ungmute ?(\d+)?"))
async def endgmute(event):
    ...
    try:
        ...
        await eor(event, "Successfully ungmuted that Noob")
