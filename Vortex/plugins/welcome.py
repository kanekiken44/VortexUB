...
from Vortex.plugins.sql_helper.welcome_sql import (
    add_welcome_setting,
...
@Vortex.on(admin_cmd(pattern="savewelcome"))  # pylint:disable=E0602
@Vortex.on(sudo_cmd(pattern="savewelcome", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@Vortex.on(admin_cmd(pattern="clearwelcome"))  # pylint:disable=E0602
@Vortex.on(sudo_cmd(pattern="clearwelcome", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    cws = get_current_welcome_settings(event.chat_id)
...
@Vortex.on(admin_cmd(pattern="listwelcome"))  # pylint:disable=E0602
@Vortex.on(sudo_cmd(pattern="listwelcome", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    cws = get_current_welcome_settings(event.chat_id)
