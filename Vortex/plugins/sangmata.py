...
@Vortex.on(admin_cmd(pattern="sg ?(.*)"))
@Vortex.on(sudo_cmd(pattern="sg ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    ...
    async with event.client.conversation(chat) as conv:
        ...
        if response1.text.startswith("No records found"):
            ...
            await event.delete()
            await Vortexuserbot.send_message(event.chat_id, response2.message)

            await Vortexuserbot.send_message(event.chat_id, response3.message)
