...
@Vortex.on(admin_cmd(outgoing=True, pattern=r"go (.*)"))
@Vortex.on(sudo_cmd(allow_sudo=True, pattern=r"go (.*)"))
...
@Vortex.on(admin_cmd("duckduckgo (.*)"))
@Vortex.on(sudo_cmd("duckduckgo (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@Vortex.on(admin_cmd(pattern="ggl (.*)"))
@Vortex.on(sudo_cmd(pattern="ggl (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    input_str = event.pattern_match.group(1)
    ...
    if response_api:
        await eor(
            event,
            "[{}]({})\n`Spoonfeeding you kid. Vortexuserbot🖤️` ".format(input_str, response_api.rstrip()),
        )
    else:
