...
from Vortex.utils import admin_cmd, humanbytes

MAX_MESSAGE_SIZE_LIMIT = 4095


@Vortex.on(admin_cmd(outgoing=True, pattern=r"ls ?(.*)"))
@Vortex.on(sudo_cmd(allow_sudo=True, pattern=r"ls ?(.*)"))
async def lst(event):
    if event.fwd_from:
        return
    vortex = event.pattern_match.group(1)
    if vortex:
        path = vortex
    else:
        path = os.getcwd()
    if not exists(path):
        await eor(
            event,
            f"There is no such directory or file with the name `{vortex}` check again!",
        )
        return
    if isdir(path):
        if vortex:
