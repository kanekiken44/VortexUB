...
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "Vortexuserbot"
DEFAULTUSERBIO = "Error : No bio found !!"
...
@Vortex.on(admin_cmd(pattern="clone ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
...
@Vortex.on(admin_cmd(pattern="revert$"))
async def _(event):
    if event.fwd_from:
        return
    name = f"{DEFAULTUSER}"
