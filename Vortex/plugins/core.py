...
from Vortex.utils import admin_cmd, load_module, remove_plugin
...
@Vortex.on(admin_cmd(pattern=r"send (?P<shortname>\w+)", outgoing=True))
@Vortex.on(sudo_cmd(pattern=r"send (?P<shortname>\w+)", allow_sudo=True))
async def send(event):
    ...
    the_plugin_file = "./Vortex/plugins/{}.py".format(input_str)
...
@Vortex.on(admin_cmd(pattern="install"))
        ...
        try:
            downloaded_file_name = (
                await event.client.download_media(  # pylint:disable=E0602
                    ...
                    "Vortex/plugins/",  # pylint:disable=E0602
                )
            )
            if "(" not in downloaded_file_name:
                ...
                await event.edit(
                    "Vortexuserbot Succesfully Installed The Plugin `{}`".format(
...
@Vortex.on(admin_cmd(pattern=r"unload (?P<shortname>\w+)$"))
async def unload(event):
    ...
    try:
        ...
        await event.edit(f"Vortexuserbot has successfully unloaded {shortname}")
    except Exception as e:
        await event.edit(
            "Vortexuserbot has successfully unloaded {shortname}\n{}".format(
                shortname, str(e)
            )
        )


@Vortex.on(admin_cmd(pattern=r"load (?P<shortname>\w+)$"))
async def load(event):
    ...
    try:
        ...
        await event.edit(f"Vortexuserbot has successfully loaded {shortname}")
    except Exception as e:
        await event.edit(
            f"Vortexuserbot could not load {shortname} because of the following error.\n{str(e)}"
        )


@Vortex.on(admin_cmd(pattern=r"installall$"))
async def install(event):
    if event.fwd_from:
        return
    ...
    for ixo in total_doxx:
        ...
        downloaded_file_name = await event.client.download_media(
            await event.client.get_messages(event.chat_id, ids=mxo), "Vortex/plugins/"
        )
        if "(" not in downloaded_file_name:
            path1 = Path(downloaded_file_name)
            shortname = path1.stem
