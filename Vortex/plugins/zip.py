...
@Vortex.on(admin_cmd(pattern="zip"))
@Vortex.on(sudo_cmd(pattern="zip", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@Vortex.on(admin_cmd(pattern="unzip"))
@Vortex.on(sudo_cmd(pattern="unzip", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    mone = await eor(event, "Processing ...")
...
CMD_HELP.update(
    {
        "zip_unzip": ".zip <reply to file>\nUse - Compress-zips the replied file.\
        .unzip <reply to a zip>\nUse - Uncompresses-Unzips the Zip File."
    }
)
