...
@Vortex.on(admin_cmd(pattern=r"(set|get|del) var (.*)", outgoing=True))
...
@Vortex.on(admin_cmd(pattern="usage"))
@Vortex.on(sudo_cmd(pattern="usage", allow_sudo=True))
async def dyno_usage(dyno):
    ...
    return await dyno.edit(
        ...
        f"     𒊹︎︎︎  `{AppHours}`**h**  `{AppMinutes}`**m**  "
        f"**|**  [`{AppPercentage}`**%**]"
        "\n\n"
        " -> `Dyno hours quota remaining this month`:\n"
        f"     𒊹︎︎︎  `{hours}`**h**  `{minutes}`**m**  "
        f"**|**  [`{percentage}`**%**]"
    )


@Vortex.on(admin_cmd(pattern="info heroku"))
@Vortex.on(sudo_cmd(pattern="info heroku", allow_sudo=True))
...
@Vortex.on(admin_cmd(outgoing=True, pattern=r"logs"))
@Vortex.on(sudo_cmd(allow_sudo=True, pattern=r"logs"))
async def _(givelogs):
    ...
    with open("logs-Vortexuserbot.txt", "w") as log:
    ...
    await givelogs.client.send_file(
        givelogs.chat_id,
        "logs-Vortexuserbot.txt",
        reply_to=givelogs.id,
        caption=f"**Your** Vortexuserbot Logs.\nPasted [here]({url}) too!",
    )
    await eor(givelogs, "Heroku Logs Incoming!!")
    await asyncio.sleep(5)
    await givelogs.delete()
    return os.remove("logs-Vortexuserbot.txt")
