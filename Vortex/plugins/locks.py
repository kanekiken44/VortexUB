...
from Vortex.plugins.sql_helper.locks_sql import get_locks, is_locked, update_lock
from Vortex.utils import admin_cmd


@Vortex.on(admin_cmd(pattern=r"lock( (?P<target>\S+)|$)"))
@Vortex.on(sudo_cmd(pattern=r"lock( (?P<target>\S+)|$)", allow_sudo=True))
async def _(event):
...
@Vortex.on(admin_cmd(pattern="unlock ?(.*)"))
@Vortex.on(sudo_cmd(pattern="unlock ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@Vortex.on(admin_cmd(pattern="currenabledlocks"))
@Vortex.on(sudo_cmd(pattern="currenabledlocks", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    res = ""
...
@Vortex.on(events.MessageEdited())  # pylint:disable=E0602
@Vortex.on(events.NewMessage())  # pylint:disable=E0602
...
@Vortex.on(events.ChatAction())  # pylint:disable=E0602
async def _(event):
    # TODO: exempt admins from locks
    # check for "lock" "bots"
