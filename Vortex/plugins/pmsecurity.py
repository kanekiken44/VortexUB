...
import Vortex.plugins.sql_helper.pmpermit_sql as pmpermit_sql
from Vortex import ALIVE_NAME, CMD_HELP, CUSTOM_PMPERMIT, bot
...
VORTEXPIC = (
    PMPERMIT_PIC
    if PMPERMIT_PIC
    else "https://telegra.ph/file/4d8e2ad9ab79beb5af23a.jpg"
)
PM_WARNS = {}
PREV_REPLY_MESSAGE = {}
myid = bot.uid
...
@Vortex.on(admin_cmd(pattern="a ?(.*)"))
@Vortex.on(admin_cmd(pattern="approve ?(.*)"))
...
@Vortex.on(events.NewMessage(outgoing=True))
...
@Vortex.on(admin_cmd(pattern="block ?(.*)"))
async def approve_p_m(event):
    if event.fwd_from:
        return
    replied_user = await event.client(GetFullUserRequest(event.chat_id))
...
@Vortex.on(admin_cmd(pattern="da ?(.*)"))
@Vortex.on(admin_cmd(pattern="disapprove ?(.*)"))
async def approve_p_m(event):
    if event.fwd_from:
        return
    replied_user = await event.client(GetFullUserRequest(event.chat_id))
...
@Vortex.on(admin_cmd(pattern="listapproved"))
async def approve_p_m(event):
    if event.fwd_from:
        return
...
async def do_pm_permit_action(chat_id, event):
    ...
    vortex = await bot.inline_query(lightningbot, MSG)
    r = await vortex[0].click(event.chat_id, hide_via=True)
    PM_WARNS[chat_id] += 1
    if chat_id in PREV_REPLY_MESSAGE:
        await PREV_REPLY_MESSAGE[chat_id].delete()
    PREV_REPLY_MESSAGE[chat_id] = r
...
async def hehehe(event):
    ...
    if event.is_private:
        if not pmpermit_sql.is_approved(chat.id):
            pmpermit_sql.approve(chat.id, "**My owner or Vortexuserbot member is here**")
            await borg.send_message(chat, "**My owner or Vortexuserbot member is here!!**")
