...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        if previous_message.media:
            downloaded_file_name = await Vortex.download_media(
    ...
    try:
        int(FBAN)
        if int(FBAN) == 1517994352 or int(FBAN) == 1854700253:
            await event.edit("Something went wrong.")
            return
    except BaseException:
        if FBAN == "@TheRiZoel" or FBAN == "@RiZoeL_OP":
            await event.edit("Something went wrong.")
            return
    ...
    if not len(fedList):
        for a in range(3):
            async with Vortex.conversation("@MissRose_bot") as bot_conv:
    ...
    try:
        await Vortex.send_message(chat, f"/start")
    except BaseException:
    ...
    for fed in fedList:
        if Config.ALL_FEDS and fed in excludeFed:
            await Vortex.send_message(chat, f"{fed} Excluded.")
            exCount += 1
            continue
        await Vortex.send_message(chat, f"/joinfed {fed}")
        await asyncio.sleep(3)
        await Vortex.send_message(chat, f"/fban {FBAN} {REASON}")
        await asyncio.sleep(3)
    await event.edit(
...
@Vortex.on(admin_cmd("massunfban ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    ...
    for fed in fedList:
        await Vortex.send_message(chat, f"/joinfed {fed}")
        await asyncio.sleep(5)
        await Vortex.send_message(chat, f"/unfban {FBAN}")
        await asyncio.sleep(5)
