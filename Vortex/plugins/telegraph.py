...
@Vortex.on(admin_cmd(pattern="telegraph (media|text) ?(.*)"))
@Vortex.on(sudo_cmd(pattern="telegraph (media|text) ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    ...
    if event.reply_to_msg_id:
        ...
        if input_str == "media":
            ...
            try:
                ...
                os.remove(downloaded_file_name)
                await okey.edit(
                    "Uploaded [here](https://telegra.ph{}) in {} seconds by RiZoeL UserBot.".format(
