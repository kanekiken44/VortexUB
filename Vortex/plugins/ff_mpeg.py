...
@Vortex.on(admin_cmd(pattern="ffmpegsave"))
@Vortex.on(sudo_cmd(pattern="ffmpegsave", allow_sudo=True))
...
@Vortex.on(admin_cmd(pattern="ffmpegtrim"))
@Vortex.on(sudo_cmd(pattern="ffmpegtrim", allow_sudo=True))
async def ff_mpeg_trim_cmd(event):
    if event.fwd_from:
        return
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
