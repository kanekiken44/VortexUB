...
@Vortex.on(admin_cmd(pattern="botgban ?(.*)"))
@Vortex.on(sudo_cmd(pattern="botgban ?(.*)", allow_sudo=True))
async def _(event):
...
@Vortex.on(admin_cmd(pattern="botungban ?(.*)"))
@Vortex.on(sudo_cmd(pattern="botungban ?(.*)", allow_sudo=True))
async def _(event):
    if Config.G_BAN_LOGGER_GROUP is None:
        await eor(
            event,
