...
@Vortex.on(admin_cmd(pattern="sudo"))
async def sudo(event):
    sudo = "True" if Config.SUDO_USERS else "False"
    users = os.environ.get("SUDO_USERS", None)
    if sudo == "True":
        await eor(event, f"**Vortex userbot**\nSudo - `Enabled`\nSudo user(s) - `{users}`")
    else:
        await eor(event, f"**Vortex Userbot**\nSudo - `Disabled`")


@Vortex.on(admin_cmd(pattern="prefix"))
...
@Vortex.on(admin_cmd(pattern="addsudo(?: |$)"))
async def tb(event):
    ok = await eor(event, "Adding user as a sudo...")
    Vortexuserbot = "SUDO_USERS"
    ...
    heroku_var[Vortexuserbot] = newsudo
