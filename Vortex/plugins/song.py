...
@Vortex.on(admin_cmd(pattern="song (.*)"))
async def download_video(vortex):
    x = await eor(vortex, "Searching...")
    url = vortex.pattern_match.group(1)
    if not url:
    ...
    try:
        ...
        await Vortexuserbot(JoinChannelRequest(sung))
    except BaseException:
        pass
    theupload = """
Uploading...
...
@Vortex.on(admin_cmd(pattern="vsong (.*)"))
async def download_video(vortex):
    x = await eor(vortex, "Processing..")
    url = vortex.pattern_match.group(1)
    if not url:
    ...
    try:
        ...
        pass
    theupload = """
Uploading Say thanks to Vortexuserbot️...
Song name - {}
By - {}
""".format(
        rip_data["title"], rip_data["uploader"]
