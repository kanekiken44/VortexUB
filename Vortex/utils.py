...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        # support for uniborg
        sys.modules["uniborg.util"] = Vortex.utils
        mod.Config = Config
        mod.borg = bot
        mod.Vortex = bot
        # auto-load
        ...
        sys.modules["Vortex.events"] = Vortex.utils
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["Vortex.plugins." + shortname] = mod
        print("Successfully (re)imported " + shortname)
...
def start_mybot(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"Vortex/plugins/assistant/{shortname}.py")
        name = "Vortex.plugins.assistant.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        print("Initialising Assistant.")
        print("Assistant - Imported " + shortname)
    else:
        import importlib
        import sys
        from pathlib import Path
        ...
        mod.tgbot = bot.tgbot
        spec.loader.exec_module(mod)
        sys.modules["Vortex.plugins.assistant" + shortname] = mod
...
def load_pmbot(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"Vortex/plugins/assistant/pm/{shortname}.py")
        name = "Vortex.plugins.assistant.pm.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        mod.tgbot = bot.tgbot
        spec.loader.exec_module(mod)
        sys.modules["Vortex.plugins.assistant.pm." + shortname] = mod
